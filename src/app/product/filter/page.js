...
export default function ProductFilter() {
  ...
  const { data: productData } = useFetch(`${process.env.NEXT_PUBLIC_SECURE_SHOP_DOMAIN}/products/filter?str=${searchStr}&_limit=20`, "application/json", user.token);
  const { data: shopData } = useFetch(`${process.env.NEXT_PUBLIC_SECURE_SHOP_DOMAIN}/shops/filter?str=${searchStr}&_limit=20`, "application/json", user.token);
        ...
        <div className='m-2'>
          <div className='row'>
            {shopsFilter && shopsFilter.map((x) => (
              <div className='col-lg-3 col-md-6' key={x.userEmail}>
                <ShopC sourceImg={!x.image || x.image == "string" ? '/sample2.png' : `data:image/jpeg/png/jpg;base64, ${x.image}`} shopName={x.shopName} email={x.userEmail}/>
              </div>
            ))}
          </div>
        </div>
